...
class TakeOff_Task():
    ...
    def get_reward(self):
        ...
        # reward = zero for matching target z, more as you go farther up to 10 
        reward = min(-(abs(self.target_pos[2] - self.sim.pose[2]) + 1), 10)/10
        
        if self.sim.time > self.sim.runtime:  # extra penalty for exceeeding time
            reward -= 5.0  #
            self.done = True
